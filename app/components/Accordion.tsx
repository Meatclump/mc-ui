...
const AccordionTrigger = React.forwardRef<React.ElementRef<typeof rxAccordion.Trigger>, React.ComponentPropsWithoutRef<typeof rxAccordion.Trigger>>(({
	...
	return (
		<rxAccordion.Header className="flex">
			<rxAccordion.Trigger
				ref={forwardRef}
				className={cn("bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-300 px-3 py-1 flex-1 flex justify-between items-center [&[data-state=open]>svg]:rotate-180 transition-all", className)}
				{...props}
			>
				{children}
